...
add_modrinth_mod(){
    ...
    if [[ -z "$project_info" ]]; then
        print_error "Failed to fetch mod info for '$slug'"; return 1
    fi
...
download_modrinth_mod(){
    ...
    if [[ "$version_file" == "null" ]] || [[ -z "$version_file" ]]; then
        print_error "No compatible version found for Minecraft $mc_version with $mod_loader"; return 1
    fi
    ...
    if [[ -z "$download_url" ]]; then
        print_error "Failed to get download URL"; return 1
    fi
    ensure_dir "$output_dir"
    local output_file="$output_dir/$filename"
    if [[ -f "$output_file" ]]; then
        print_info "Already downloaded: $filename"; return 0
    fi
...
upgrade_all(){
    ...
    $JSON_PROC -c '.mods[]' < "$profile_file" | while IFS= read -r mod; do
        ...
        case "$source" in
            ...
            curseforge) print_error "CurseForge downloads not yet implemented (requires API key)";;
...
remove_mod(){
    ...
    if [[ -z "$identifier" ]]; then
        print_error "Usage: $0 remove <slug|id>"; return 1
    fi
...
case "$COMMAND" in
    ...
    *)
        if [[ -z "$COMMAND" ]]; then
            ...
            print_error "Unknown command: $COMMAND"; echo ""; show_help
        fi; exit 1;;
esac
