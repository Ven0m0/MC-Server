...
# ZGC-optimized configuration (removed conflicting G1GC flags and duplicates)
sudo java -Xms8192M -Xmx8192M \
  ...
  -XX:+UseCharacterCompareIntrinsics -XX:+UseCopySignIntrinsic \
  -XX:+UseThreadPriorities -XX:+UseAES -XX:+UseAESIntrinsics \
  -XX:LargePageSizeInBytes=2M -XX:+UseLargePages \
  -XX:+EagerJVMCI -XX:+UseNUMA -XX:+UseFastStosb \
  -XX:+RewriteBytecodes -XX:+RewriteFrequentPairs \
  -XX:+UseXMMForArrayCopy -XX:+UseXmmI2D -XX:+UseXmmI2F \
  -XX:+UseXmmRegToRegMoveAll -XX:+UseXmmLoadAndClearUpper \
  -XX:+DoEscapeAnalysis -XX:+AlignVector \
