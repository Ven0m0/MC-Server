...
- Python-first Minecraft Fabric server automation suite.
- Main code lives in `tools/*.py`.
...
- Follow the Python conventions in `AGENTS.md`: type hints, `pathlib`, and 4-space indentation.
- Reuse helpers from `tools/common.py` for logging, dependency checks, downloads, and server detection.
...
- `python3 -m py_compile tools/*.py minecraft/config/versions.py`
- `ruff check tools/ minecraft/config/versions.py` (if ruff installed)
