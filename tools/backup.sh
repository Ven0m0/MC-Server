#!/usr/bin/env bash
# backup.sh: Simplified Minecraft server backup tool
...
ensure_dir "${BACKUP_DIR}/worlds"
ensure_dir "${BACKUP_DIR}/configs"
...
create_btrfs_snapshot() {
  ...
  [[ ! -d $source ]] && { print_error "Source directory not found: ${source}"; return 1; }
  is_btrfs "$source" || { print_error "Source is not on Btrfs filesystem"; print_info "Use regular backup instead"; return 1; }
  has_command btrfs || { print_error "btrfs command not found"; return 1; }
  ...
  if ((EUID == 0)); then
    ...
    sudo btrfs subvolume snapshot -r "$source" "$snapshot_path" || { print_error "Failed to create snapshot (root access required)"; return 1; }
  fi
...
list_btrfs_snapshots() {
  local snapshot_dir="${BACKUP_DIR}/btrfs-snapshots"
  [[ ! -d $snapshot_dir ]] && { print_info "No Btrfs snapshots found"; return 0; }
  ...
  if ((EUID == 0)); then
    btrfs subvolume list "$snapshot_dir" 2>/dev/null || find "$snapshot_dir" -maxdepth 1 -type d -name "snapshot_*" -printf '%f\n' | sort
  else
...
delete_btrfs_snapshot() {
  ...
  [[ ! -d $snapshot_path ]] && { print_error "Snapshot not found: ${snapshot_name}"; return 1; }
  has_command btrfs || { print_error "btrfs command not found"; return 1; }
  ...
  if ((EUID == 0)); then
    ...
    sudo btrfs subvolume delete "$snapshot_path" || { print_error "Failed to delete snapshot (root access required)"; return 1; }
  fi
...
restore_btrfs_snapshot() {
  ...
  if ((EUID == 0)); then
    ...
    sudo btrfs subvolume snapshot "$snapshot_path" "$target" || { print_error "Failed to restore snapshot (root access required)"; return 1; }
  fi
