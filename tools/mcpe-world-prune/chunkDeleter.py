...
class deleteZoneWindow:
    def __init__(self, master, deleteque):
        ...
        Entry(master=top, textvariable=self.nameVar, width=37, borderwidth=1).grid(
            row=r, column=2, columnspan=4
        )
        r += 1
        ...
        Entry(self.top, textvariable=self.XMax, borderwidth=1, width=5).grid(
            row=r, column=2
        )
        Entry(self.top, textvariable=self.XMin, borderwidth=1, width=5).grid(
            row=r, column=3
        )
        Entry(self.top, textvariable=self.ZMax, borderwidth=1, width=5).grid(
            row=r, column=4
        )
        Entry(self.top, textvariable=self.ZMin, borderwidth=1, width=5).grid(
            row=r, column=5
        )
        r += 1
        Checkbutton(self.top, text="Overworld", variable=self.overworld).grid(
            row=r, column=1
        )
        r += 1
    ...
    def cleanup(self):
        ...
        if self.overworld.get():
            deleteVal[overworld] = {}
            deleteVal[overworld][self.nameVar.get()] = {
                "xstart": xmin,
                "xstop": xmax,
                "zstart": zmin,
                "zstop": zmax,
        ...
        if self.nether.get():
            deleteVal[nether] = {}
            deleteVal[nether][self.nameVar.get()] = {
                "xstart": xmin,
                "xstop": xmax,
                "zstart": zmin,
                "zstop": zmax,
        ...
        if self.theEnd.get():
            deleteVal[theEnd] = {}
            deleteVal[theEnd][self.nameVar.get()] = {
                "xstart": xmin,
                "xstop": xmax,
                "zstart": zmin,
                "zstop": zmax,
...
class mainWindow:
    def __init__(self, master):
        ...
        self.save = {overworld: {self.OWXMax.get()}, nether: {}, theEnd: {}}

        r = 0

        Label(self.master, text="World File", borderwidth=1).grid(row=r, column=1)
        Entry(self.master, textvariable=self.worldFile, borderwidth=1, width=40).grid(
            row=r, column=2, columnspan=3
        )
        Button(
            self.master, text="Browse", command=self.browseWorld, borderwidth=1
        ).grid(row=r, column=5)
        r += 1
        ...
        Entry(self.master, textvariable=self.OWXMin, borderwidth=1, width=6).grid(
            row=r, column=2
        )
        Entry(self.master, textvariable=self.OWXMax, borderwidth=1, width=6).grid(
            row=r, column=3
        )
        Entry(self.master, textvariable=self.OWZMin, borderwidth=1, width=6).grid(
            row=r, column=4
        )
        Entry(self.master, textvariable=self.OWZMax, borderwidth=1, width=6).grid(
            row=r, column=5
        )
        r += 1
        Label(self.master, text="Nether", borderwidth=1).grid(row=r, column=1)
        Entry(self.master, textvariable=self.NeXMin, borderwidth=1, width=6).grid(
            row=r, column=2
        )
        Entry(self.master, textvariable=self.NeXMax, borderwidth=1, width=6).grid(
            row=r, column=3
        )
        Entry(self.master, textvariable=self.NeZMin, borderwidth=1, width=6).grid(
            row=r, column=4
        )
        Entry(self.master, textvariable=self.NeZMax, borderwidth=1, width=6).grid(
            row=r, column=5
        )

        r += 1
        Label(self.master, text="End", borderwidth=1).grid(row=r, column=1)
        Entry(self.master, textvariable=self.EndXMin, borderwidth=1, width=6).grid(
            row=r, column=2
        )
        Entry(self.master, textvariable=self.EndXMax, borderwidth=1, width=6).grid(
            row=r, column=3
        )
        Entry(self.master, textvariable=self.EndZMin, borderwidth=1, width=6).grid(
            row=r, column=4
        )
        Entry(self.master, textvariable=self.EndZMax, borderwidth=1, width=6).grid(
            row=r, column=5
        )
        ...
        self.addButton = Button(
            self.master, text="Add Delete Zone", command=lambda: self.addDeleteZone()
        )
        self.addButton.grid(row=r, column=5)
        r += 1
        self.delButton = Button(
            self.master, text="Remove Delete Zone", command=self.deleteSkin
        ).grid(row=r, column=5)
        r += 1
        self.delButton = Button(
            self.master, text="Export", command=self.pruneWorld
        ).grid(row=r, column=5)
    ...
    def browseWorld(self):
        self.worldFile.set(
            filedialog.askopenfilename(
                filetypes=(("World files", "*.mcworld"), ("World Files", "*.zip"))
            )
        )
    ...
    def pruneWorld(self):
        ...
        self.save = {overworld: {}, nether: {}, theEnd: {}}
        self.save[overworld] = {
            "xstart": min(owx),
            "xstop": max(owx),
            "zstart": min(owz),
        ...
        with bedrock.World(path_to_save) as world:
            for dim in [overworld, nether, theEnd]:
                saLim = self.save[dim]
                print(saLim)
                for blockX in np.arange(
                    start=worldExtense[0][0], stop=worldExtense[0][1], step=16
                ):
                    for blockZ in np.arange(
                        start=worldExtense[1][0], stop=worldExtense[1][1], step=16
                    ):
                        chunkX = blockX // 16
                        chunkZ = blockZ // 16
                        ...
                        if (
                            saLim["zstart"] < blockZ
                            and saLim["zstop"] > blockZ
                            and saLim["xstart"] < blockX
                            and saLim["xstop"] > blockX
                        ):
                            pass
                        else:
                        ...
                        for name in self.delete[dim].keys():
                            ...
                            if (
                                delLim["zstart"] < blockZ
                                and delLim["zstop"] > blockZ
                                and delLim["xstart"] < blockX
                                and delLim["xstop"] > blockX
                            ):
                                deleteChunk(world, chunkX, chunkZ, dim)
        startDir = os.getcwd()
        ...
        ##pack up world
        shutil.move(
            os.path.join(os.getcwd(), outputFileName + ".mcworld"),
