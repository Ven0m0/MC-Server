...
class World:
    ...
    def getChunk(self, x, z, layer=0, dim=0):
        chunk = self.chunks.get((x, z, dim), None)
        if chunk is None:
            chunk = Chunk(self.db, x, z, dim)
            self.chunks[(x, z, dim)] = chunk
        return chunk

    def getBlock(self, x, y, z, layer=0, dim=0):
        cx = x // 16
        x %= 16
        cz = z // 16
        z %= 16
        chunk = self.getChunk(cx, cz, dim=dim)
        return chunk.getBlock(x, y, z, layer, dim=dim)
...
class Chunk:
    def __init__(self, db, x, z, dim=0):
        if dim == 0:
            dimention = bytearray()
        else:
            dimention = struct.pack("<i", dim)

        self.x = x
        self.z = z
        # Leveldb chunks are stored in a number of keys with the same prefix.
        self.keyBase = struct.pack("<ii", self.x, self.z) + dimention
        ...
        for i in range(16):
            try:
                self.subchunks.append(
                    SubChunk(db, self.x, self.z, i, dim=dim)
                )  # Pass off processing to the subchunk class
        ...
        try:
            ...
            if version not in [10, 13, 14, 15]:
                raise NotImplementedError(
                    "Unexpected chunk version {} at chunk {} {}.".format(
                        version, self.x, self.z
                    )
                )
        except KeyError:
    ...
    def _loadTileEntities(self, db):
        ...
        while not data.finished():
            ...
            x = (
                nbtData.pop("x").payload
            )  # We add back theses with the correct value on save, they are important.
            y = nbtData.pop("y").payload
            z = nbtData.pop("z").payload
            block = self.getBlock(x % 16, y, z % 16)
            if not block:
                print(
                    "Warning: Cannot apply nbt to block at {} {} {} since it does not exist.".format(
                        x, y, z
                    )
                )
                continue
    ...
    def delete(self, db):
        ldb.delete(db, self.keyBase + b"v")
        ldb.delete(db, self.keyBase + b"0")
        ldb.delete(db, self.keyBase + b"1")
        ldb.delete(db, self.keyBase + b"2")
        ldb.delete(db, self.keyBase + b"4")
        ldb.delete(db, self.keyBase + b"5")
        ldb.delete(db, self.keyBase + b"6")
        ldb.delete(db, self.keyBase + b"7")
        ldb.delete(db, self.keyBase + b"8")
        ldb.delete(db, self.keyBase + b"9")
        ldb.delete(db, self.keyBase + b"-")
        ldb.delete(db, self.keyBase + b".")
        ldb.delete(db, self.keyBase + b":")
        ldb.delete(db, self.keyBase + b"?")
...
class SubChunk:
    def __init__(self, db, x, z, y, dim=0):
        if dim == 0:
            dim = bytearray()
        else:
            dim = struct.pack("<i", dim)
        self.dirty = False
        self.x = x
        self.z = z
        self.y = y
        if db is not None:  # For creating subchunks, there will be no DB.
            ...
            key = struct.pack("<ii", x, z) + dim + struct.pack("<cB", b"/", y)
            try:
                data = ldb.get(db, key)
            except KeyError:
            ...
            if self.version != 8:
                raise NotImplementedError(
                    "Unsupported subchunk version {} at {} {}/{}".format(
                        self.version, x, z, y
                    )
                )
            ...
            for i in range(numStorages):
                ...
                self.blocks.append(
                    np.empty(4096, dtype=Block)
                )  # Prepare with correct dtype
                for j, block in enumerate(blocks):
                    ...
                    try:  # 1.13 format
                        ...
                        self.blocks[i][j] = Block(
                            block["name"].payload, block["states"].payload
                        )  # .payload to get actual val
                    except KeyError:  # 1.12 format
                        self.blocks[i][j] = Block(
                            block["name"].payload, block["val"].payload
                        )  # .payload to get actual val
                self.blocks[i] = (
                    self.blocks[i].reshape(16, 16, 16).swapaxes(1, 2)
                )  # Y and Z saved in an inverted order
    ...
    def getBlock(self, x, y, z, layer=0):
        if layer >= len(self.blocks):
            raise KeyError(
                "Subchunk {} {}/{} does not have a layer {}".format(
                    self.x, self.z, self.y, layer
                )
            )
    ...
    def _saveBlocks(self, paletteSize, blockIDs):
        ...
        for bits in [1, 2, 3, 4, 5, 6, 8, 16]:
            ...
            raise NotImplementedError(
                "Too many bits per block needed {} at {} {}/{}".format(
                    bitsPerBlock, self.x, self.z, self.y
                )
            )
    ...
    def _savePalette(self, layer):
        blocks = (
            self.blocks[layer].swapaxes(1, 2).reshape(4096)
        )  # Y and Z saved in a inverted order
                    ...
                    palette.append(
                        nbt.TAG_Compound(
                            ...
                            [
                                ...
                                nbt.TAG_Short("val", block.properties),
                            ],
                        )
                    )
                else:  # 1.13
...
class CommandBlock(Block):
    ...
    def __init__(
        ...
        nbtData.add(
            nbt.TAG_Int("LPCommandMode", 0)
        )  # Not sure what these LPModes do. This works.
